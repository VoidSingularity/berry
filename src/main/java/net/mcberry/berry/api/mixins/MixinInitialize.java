...
import net.mcberry.berry.utils.Graph;
...
public class MixinInitialize {
    public static void initialize() {
        ...
        BerryClassTransformer.ByteCodeTransformer transformer = (loader, name, clazz, domain, code) -> {
            try {
                name = name.replace('/', '.');
                if (name.startsWith("org.spongepowered.") && !name.startsWith("org.spongepowered.asm.synthetic"))
                    return code;
    ...
    public static void addcfg() {
        Mixins.addConfiguration("META-INF/berry/builtin_mixins.json");
    }
}
